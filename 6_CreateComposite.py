    ...
    with open(gdbRegions_dict, 'rU') as inputFile2:
        for line in inputFile2:
            ...
            if L48:
                ...
                FeatureType = "Polygon"
                wildcard = str(FileType) + "*"
                print wildcard
                for value in pdate:
                    Comp_FileName = regionname + filetype + group + "_L48_" + projection + "_" + str(value)
                ...
                if pcreated:
                    continue
                Comp_FileName = regionname + filetype + group + "_L48_" + projection + "_" + str(date)
                filepath = outGDB + os.sep + Comp_FileName
                print filepath
                if arcpy.Exists(filepath):
                    continue
                    ...
                    if len(fcList) == 0:
                        ...
                        arcpy.Merge_management(fcList, filepath, fms)
                        arcpy.env.workspace = outGDB
                        arcpy.env.overwriteOutput = True
                        # completetable = str(Comp_FileName) + "_table"
                        # arcpy.CopyRows_management(filepath, completetable)
                ...
                if regionname == "Lower48Only":
                    continue
                FileType = regionname + "_"
                if regionname == "PartialLower48":
                    FileType = "Lower48_"
                arcpy.env.workspace = regionsgdb
                arcpy.env.overwriteOutput = True
                ...
                print wildcard
                for value in pdate:
                    Comp_FileName = regionname + filetype + group + "_NL48_" + projection + "_" + str(value)
                    filepath = outGDB + os.sep + Comp_FileName
                    print filepath
                    if arcpy.Exists(filepath):
                        pcreated = True
                ...
                if pcreated == True:
                    continue
                Comp_FileName = regionname + filetype + group + "_NL48_" + projection + "_" + str(date)
                filepath = outGDB + os.sep + Comp_FileName
                print filepath
                if arcpy.Exists(filepath):
                    continue
                    ...
                    if len(fcList) == 0:
                        ...
                        arcpy.Merge_management(fcList, filepath)
                        arcpy.env.workspace = outGDB
                        arcpy.env.overwriteOutput = True
                        # completetable = str(Comp_FileName) + "_table"
                        # arcpy.CopyRows_management(filepath, completetable)
